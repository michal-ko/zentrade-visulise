...
if downsample:
    ...
    filtered_df = downsampled_df[
        downsampled_df["timestamp"] > pd.Timestamp(selected_start_dt, tz=TIMEZONE)
    ]
else:
...
if chart_option == "Individual metric":
    selected_columns = st.sidebar.multiselect("Select metrics:", COLUMNS, default=["close"])
    st.line_chart(filtered_df, x="timestamp", x_label="date", y=selected_columns, y_label="price")
